...
function sessionDetail(step: ConsoleStep) {
  ...
  if (step === "template-match" || step === "drafting") return "Catalog match · plan binding";
...
export function Topbar({
  ...
  const titleMap: Record<ConsoleStep, [string, string]> = {
    ...
    "template-match": ["Context resolved", "EvoPilot 自动匹配模板，用户不需要手动选择 Harness。"],
    drafting: ["selectedHarness planning", "EvoPilot 读取已发布 Catalog，并在 goal plan 中绑定 selectedHarness。"],
    ...
    activated: ["Plan ready", "selectedHarness 绑定后续 planning 与 loop execution。"],
  ...
  return (
    <div className="card">
      ...
      <div className="card-body metrics three">
        ...
        <Metric label="Owner gate" value="Required" note="plan approval needs confirmation" />
      </div>
    </div>
  );
}

function TemplateCard({ profileDraft }: { profileDraft: HarnessBindingDraft }) {
    ...
    <div className="card">
      <div className="card-head">
        <div>
          ...
          <span>接入项目不手选模板，EvoPilot 根据项目上下文和目标从已发布 Catalog 自动匹配。</span>
        </div>
        <span className="tag green">matched</span>
      </div>
      <div className="card-body metrics three">
        <Metric label="selectedHarness" value={profileDraft.templateRef || "auto-match"} note={profileDraft.domain || "runtime baseline"} />
...
function DraftingCard() {
  return (
    <div className="card">
      <div className="card-head">
        <div>
          <strong>Generating selectedHarness</strong>
          <span>读取 Catalog 并把 selectedHarness 绑定到 Alpha/Beta/RC/GA phase plan。</span>
        </div>
...
function ProfileReviewCard({ context, profileDraft }: { context: ProjectLoopContext; profileDraft: HarnessBindingDraft }) {
  return (
    <div className="card">
      <div className="card-head">
        <div>
          <strong>selectedHarness.yaml</strong>
          <span>这是用户确认前看到的人类可读 selectedHarness 计划绑定。</span>
        </div>
      ...
      <div className="card-body">
        <div className="review-document">
          <div className="doc-toolbar">
            <code>plans/{context.projectId || "project"}/selectedHarness.yaml</code>
          ...
          <div className="markdown-note">
            ...
            <ul>
              <li><b>Scope:</b> review the Catalog, Harness version, path, and digests before plan approval.</li>
              <li><b>Decision:</b> confirmation allows explicit phase plan approval; it does not change Harness definitions.</li>
              <li><b>Review focus:</b> Catalog id、Harness id、版本、entry path、catalog digest、entry digest。</li>
            </ul>
          </div>
...
function DiffCard({ profileDraft }: { profileDraft: HarnessBindingDraft }) {
  return (
    <div className="card">
      <div className="card-head">
        <div>
          <strong>Draft update diff</strong>
          <span>用户要求已触发 goal plan 重新生成，等待再次确认。</span>
        </div>
...
function ActivatedCard({ context, reviewSteps }: { context: ProjectLoopContext; reviewSteps: ReviewStep[] }) {
  return (
    <div className="card">
      <div className="card-head">
        <div>
          <strong>selectedHarness binding</strong>
          <span>从这里开始，loop execution 不能绕过 selectedHarness 契约。</span>
        </div>
...
function ReleaseCard({ profileDraft }: { profileDraft: HarnessBindingDraft }) {
  return (
    <div className="card">
      <div className="card-head">
        <div>
      ...
      <div className="card-body">
        <div className="metrics three">
          <Metric label="Harness binding" value="Ready" note="Catalog and entry digests recorded" />
        ...
        <div className="capability-list">
          <Capability text="异常处理、日志、监控、APM、CI/CD、回滚全部满足 selectedHarness binding" />
