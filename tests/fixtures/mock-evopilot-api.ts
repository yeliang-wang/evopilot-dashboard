    ...
    if (method === "POST" && /\/api\/v1\/projects\/[^/]+\/harness-profiles\/generate$/.test(url.pathname)) {
      return fulfill(route, ok({
        ...
        profile: {
          ...
          version: 1,
          status: "DRAFT",
          templateRef: "python-enterprise-harness@1.2.0",
          ...
          sourceContent: [
            ...
            "  status: DRAFT",
            "  inherits:",
            "    - python-enterprise-harness@1.2.0",
            "  controls:",
            "    logging:",
            "      requiredFields: [requestId, correlationId, phaseId]",
            "    releaseGates:",
            "      required: [TargetEvidencePackage, release decision]"
          ].join("\n"),
          compiledContent: "compiled mock profile",
          generatedBy: {
            evidence: ["language=python", "framework=fastapi", "target=ga-ready"],
            selectionReasons: ["matches repository runtime and requested release gates"]
          },
          validation: {
            status: "READY",
