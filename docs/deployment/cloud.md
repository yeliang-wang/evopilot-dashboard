...
This guide covers cloud container and static hosting entrypoints for Dashboard `v3.1.2`. It does not announce a hosted EvoPilot Cloud trial or a managed SaaS product. Operators own the cloud project, DNS, TLS, secrets, EvoPilot API endpoint, and evopilot-harness Hub endpoint.
...
gcloud run deploy evopilot-dashboard \
  --image ghcr.io/yeliang-wang/evopilot-dashboard:3.1.2 \
  --port 8080 \
