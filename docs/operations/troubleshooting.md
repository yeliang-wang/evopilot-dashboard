...
2. Confirm the first page is login, or after login the page shows **Agent Console v3** with left navigation, locked scope, stage bar, conversation, composer, and evidence drawer entry.
...
| selectedHarness is missing | No configured published Harness Catalog matched the project and target | Ask an administrator to publish/configure a usable Catalog through `evopilot-harness`, then regenerate the goal plan. |
...
| Approve Phase Plan returns `409` | Plan not reviewable, missing selectedHarness evidence, or missing confirmation payload | Read Last API Action and repair the stated condition. |
...
page=<Agent Console|Tenants|Workspaces|Users|Harness Hub|LLM Profiles|Audit>
